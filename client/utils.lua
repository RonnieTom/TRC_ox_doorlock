...
local function pickLock(entity)
	...
	-- This is triggered when the player uses a lock pick.
	-- This is triggered when the player uses a lock pick.
	-- This is triggered when the player uses a lock pick.
	-- This is triggered when the player uses a lock pick.
	-- This is triggered when the player uses a lock pick.
	-- This is triggered when the player uses a lock pick.
	-- This is triggered when the player uses a lock pick.
	ExecuteCommand('ame Try to open the lock with a lock pick')
	...
	if success then
		-- This is triggered when the player successfully unlocks a lock using a lock pick.👍
		-- This is triggered when the player successfully unlocks a lock using a lock pick.👍
		-- This is triggered when the player successfully unlocks a lock using a lock pick.👍
		-- This is triggered when the player successfully unlocks a lock using a lock pick.👍
		ExecuteCommand('do Unlock successfully')
		TriggerServerEvent('ox_doorlock:setState', door.id, door.state == 1 and 0 or 1, true)
	else
		-- This is triggered when the player fails to open a lock with a lock pick.×
		-- This is triggered when the player fails to open a lock with a lock pick.×
		-- This is triggered when the player fails to open a lock with a lock pick.×
		ExecuteCommand('do Unlock failed')
	end
...
local function sendActionMessage(action)
    ExecuteCommand('me ' .. action)
end

local function useClosestDoorWithAction()
    ...
    if gameTimer - lastTriggered > 500 then
        ...
        local currentState = ClosestDoor.state
        local newState = currentState == 1 and 0 or 1
        
        if newState == 1 then
            sendActionMessage('锁上了门')
        else
            sendActionMessage('打开了门')
        end
...
local function pickLockWithAction(entity)
    local door = getDoorFromEntity(entity)
    ...
    if success then
        sendActionMessage('成功撬开了门')
        TriggerServerEvent('ox_doorlock:setState', door.id, door.state == 1 and 0 or 1, true)
    else
        sendActionMessage('尝试打开门[失败了]')
    end
...
exports('useClosestDoorWithAction', useClosestDoorWithAction)
exports('pickLockWithAction', pickLockWithAction)
